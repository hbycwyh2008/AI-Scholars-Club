# Mission 00.2 — Evidence System, GitHub, and Bohrium Readiness
...
Locate and explain the purpose of:
...
Explain why a notebook that runs only because of hidden prior state is not valid competition evidence.
...
```text
Create environment → run minimal check → restart/clean run → record versions
→ document failure/fix → commit meaningful evidence → reproduce elsewhere
```
...
From a clean folder or runtime, reproduce the repository structure and environment check without following the guided steps line by line.
